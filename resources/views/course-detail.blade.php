...
@php
    ...
    $safeKonu = trim(strip_tags(html_entity_decode((string) $konu, ENT_QUOTES | ENT_HTML5, 'UTF-8')));
@endphp
        ...
        <section class="space-y-4">
            @foreach($subCourses as $subCourse)
                @php
                    ...
                    $subKonu = (string) data_get($subCourse->lesson_payload, 'curriculum.topic', data_get($subCourse->lesson_payload, 'lesson_description', ''));
                    $safeSubKonu = trim(strip_tags(html_entity_decode($subKonu, ENT_QUOTES | ENT_HTML5, 'UTF-8')));
                    $subKazanimlar = array_values(array_filter((array) (
                        data_get($subCourse->lesson_payload, 'curriculum.outcomes')
                        ?? data_get($subCourse->lesson_payload, 'curriculum.kazanımlar')
                        ?? data_get($subCourse->lesson_payload, 'curriculum.kazanimlar')
                        ?? []
                    ), fn ($item) => trim((string) $item) !== ''));
                    $subEtkinlikler = array_values(array_filter((array) data_get($subCourse->lesson_payload, 'curriculum.activities', []), fn ($item) => trim((string) $item) !== ''));
                @endphp
                    ...
                    <article class="w-full rounded-2xl bg-white p-6 shadow-lg" style="border:1px solid #e5eef9">
                        <div class="flex flex-wrap items-start justify-between gap-4">
                            <div class="flex items-start gap-3">
                                ...
                                <h2 class="text-2xl font-bold text-gray-900">{{ $subCourse->name }}</h2>
                            </div>
                            <div class="flex flex-wrap items-center gap-2">
                                @if(auth()->check() && (auth()->user()?->hasRole('admin') || auth()->user()?->hasRole('teacher')))
                    ...
                    <div class="mt-6 space-y-6 text-gray-700">
                        @if($safeSubKonu !== '')
                            <section>
                                <h3 class="text-lg font-bold text-gray-900">Konu:</h3>
                                <p class="mt-2 text-base leading-relaxed">{{ $safeSubKonu }}</p>
                            </section>
                        @endif

                        @if(!empty($subKazanimlar))
                            <section>
                                <h3 class="text-lg font-bold text-gray-900">Kazanımlar:</h3>
                                <ul class="mt-2 list-disc space-y-1 pl-6 text-base">
