...
<head>
    ...
    <style>
        :root{
            ...
            --brand2:#0ea5e9;
            --brand3:#7c3aed;
            --shadow:0 22px 56px rgba(30,41,59,.12);
        }
        *{box-sizing:border-box}
        html,body{margin:0;min-height:100%;font-family:Inter,ui-sans-serif,system-ui,-apple-system,"Segoe UI",sans-serif;color:var(--text);background:
            radial-gradient(circle at 14% 10%, rgba(37,99,235,.12), transparent 28%),
            radial-gradient(circle at 84% 16%, rgba(14,165,233,.08), transparent 26%),
            linear-gradient(160deg,var(--bg) 0%,var(--bg2) 56%,#ffffff 100%);overflow-x:hidden}
        ...
        .page::before{width:260px;height:260px;left:-80px;top:-80px;background:rgba(37,99,235,.12)}
        .page::after{width:340px;height:340px;right:-100px;bottom:-140px;background:rgba(124,58,237,.08)}
        .grain{position:absolute;inset:0;background-image:radial-gradient(rgba(15,23,42,.05) 1px, transparent 1px);background-size:18px 18px;opacity:.22;pointer-events:none}
        ...
        .cta-secondary{background:#fff;border-color:rgba(37,99,235,.16);color:var(--brand);box-shadow:0 12px 28px rgba(37,99,235,.10)}
        .cta:hover{transform:translateY(-2px);filter:saturate(1.04)}
        .cta-primary:hover{box-shadow:0 22px 42px rgba(37,99,235,.28)}
        .cta-secondary:hover{border-color:rgba(37,99,235,.30);box-shadow:0 16px 34px rgba(37,99,235,.16)}
        .hero-card{position:relative;padding:18px;border-radius:30px;background:linear-gradient(180deg,var(--panel),rgba(255,255,255,.68));border:1px solid rgba(148,163,184,.18);box-shadow:var(--shadow);backdrop-filter:blur(14px)}
        .hero-visual{position:relative;min-height:360px;border-radius:24px;overflow:hidden;background:
            radial-gradient(circle at 25% 20%, rgba(37,99,235,.12), transparent 20%),
            radial-gradient(circle at 78% 28%, rgba(124,58,237,.08), transparent 20%),
            linear-gradient(180deg,#ffffff,#eef5ff)}
        .logo-orbit{position:absolute;inset:18px;display:grid;place-items:center}
        .logo-orbit img{width:min(100%,420px);height:auto;display:block;filter:drop-shadow(0 12px 30px rgba(37,99,235,.14))}
        .section{padding:16px 0 12px}
        .feature-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}
        .feature{padding:18px;border-radius:22px;background:rgba(255,255,255,.9);border:1px solid rgba(148,163,184,.18);box-shadow:0 14px 28px rgba(15,23,42,.07);transition:transform .2s ease,border-color .2s ease}
        .feature:hover{transform:translateY(-3px);border-color:rgba(37,99,235,.22)}
        .feature .icon{width:42px;height:42px;border-radius:14px;display:grid;place-items:center;background:linear-gradient(135deg,rgba(37,99,235,.12),rgba(14,165,233,.10));border:1px solid rgba(148,163,184,.18);margin-bottom:14px}
        ...
        .panel{padding:22px;border-radius:24px;background:rgba(255,255,255,.9);border:1px solid rgba(148,163,184,.18);box-shadow:0 14px 30px rgba(15,23,42,.07)}
        ...
        .bullet{display:flex;gap:10px;align-items:flex-start;padding:12px 14px;border-radius:16px;background:#f8fbff;border:1px solid rgba(148,163,184,.18)}
        ...
        .footer-line{display:flex;justify-content:space-between;gap:12px;align-items:center;flex-wrap:wrap;border-top:1px solid rgba(148,163,184,.18);padding-top:14px}
        .modal-backdrop{position:fixed;inset:0;display:none;align-items:center;justify-content:center;padding:16px;background:rgba(15,23,42,.45);z-index:60}
        .modal-backdrop.is-open{display:flex}
        .modal{width:min(560px,100%);border-radius:24px;padding:22px;background:#fff;border:1px solid rgba(148,163,184,.18);box-shadow:var(--shadow);color:var(--text)}
        .modal h3{margin:0 0 8px;font-size:24px}
        .modal p{margin:0 0 18px;color:var(--muted);line-height:1.7}
        .field{display:grid;gap:8px;margin-bottom:12px}
        .field label{font-size:14px;font-weight:700}
        .field input,.field textarea{width:100%;border-radius:14px;border:1px solid rgba(148,163,184,.20);padding:12px 14px;background:#fff;color:var(--text);font:inherit}
        .field textarea{min-height:120px;resize:vertical}
        .modal-actions{display:flex;justify-content:flex-end;gap:10px;flex-wrap:wrap;margin-top:14px}
        .btn-ghost{background:#fff;border-color:rgba(148,163,184,.20);color:var(--text)}
