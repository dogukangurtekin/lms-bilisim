...
<head>
    ...
    @php
        ...
        $themeMap = [
            ...
            'purple-modern' => ['bg' => '#f5f3ff', 'text' => '#111827', 'muted' => '#5b6472', 'panel' => '#ffffff', 'surface' => '#ffffff', 'border' => '#ddd6fe', 'primary' => '#7c3aed', 'secondary' => '#2563eb', 'heroStart' => '#7c3aed', 'heroEnd' => '#2563eb'],
            'green-calm' => ['bg' => '#f0fdf4', 'text' => '#052e16', 'muted' => '#4b5563', 'panel' => '#ffffff', 'surface' => '#ffffff', 'border' => '#bbf7d0', 'primary' => '#059669', 'secondary' => '#10b981', 'heroStart' => '#047857', 'heroEnd' => '#10b981'],
            'high-contrast' => ['bg' => '#000000', 'text' => '#ffffff', 'muted' => '#e5e5e5', 'panel' => '#111111', 'surface' => '#171717', 'border' => '#facc15', 'primary' => '#facc15', 'secondary' => '#ffffff', 'heroStart' => '#000000', 'heroEnd' => '#111111'],
        ];
        $theme = $themeMap[$themeKey] ?? $themeMap['default'];
        $pwaConfig = [
...
<body
    data-theme="{{ $themeKey }}"
    class="{{ auth()->user()?->role?->slug ? 'role-'.auth()->user()->role->slug : 'role-guest' }} @yield('body_class')"
    style="--app-bg:{{ $theme['bg'] }};--app-text:{{ $theme['text'] }};--app-muted:{{ $theme['muted'] }};--app-panel:{{ $theme['panel'] }};--app-surface:{{ $theme['surface'] }};--app-border:{{ $theme['border'] }};--app-primary:{{ $theme['primary'] }};--app-secondary:{{ $theme['secondary'] }};--app-hero-start:{{ $theme['heroStart'] }};--app-hero-end:{{ $theme['heroEnd'] }};"
>
@if(auth()->user()?->hasRole('student'))
<style>
.play-compact .layout{
