            ...
            @foreach($posts as $post)
                <article class="board-post">
                    <div class="board-post-head">
                        <img src="{{ $post['avatar_path'] ? (str_starts_with($post['avatar_path'], 'http') ? $post['avatar_path'] : url('/public/' . ltrim((string) $post['avatar_path'], '/'))) : \App\Support\Brand::logoUrl() }}" alt="{{ $post['first_name'] }} {{ $post['last_name'] }}">
                        <div>
