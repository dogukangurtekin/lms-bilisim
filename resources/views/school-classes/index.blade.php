...
<div class="card">
    <form method="POST" action="{{ route('classes.store') }}" class="actions classes-form" style="margin-bottom:14px;align-items:end;flex-wrap:wrap;gap:10px">
        ...
        <div class="field-wrap">
            ...
            <input name="name" value="{{ old('name') }}" placeholder="Örn: 5">
        </div>
        <div class="field-wrap">
            <label>Şube</label>
            <input name="section" value="{{ old('section') }}" placeholder="Örn: A">
        </div>
    ...
    <form id="classes-filter-form" method="GET" class="actions classes-filter" style="margin-bottom:10px;align-items:end;flex-wrap:wrap">
        <div class="field-wrap" style="min-width:220px">
            <label>Sınıf</label>
            <input id="classes-class-name" name="class_name" value="{{ $className ?? request('class_name') }}" placeholder="Sınıf adı...">
        </div>
        <div class="field-wrap">
            <label>Şube</label>
            <input id="classes-section" name="section" value="{{ $section ?? request('section') }}" placeholder="Şube...">
        </div>
    </form>
