    ...
    public function index()
    {
        ...
        $dashboard = Cache::remember('dashboard.teacher.' . ($user?->id ?? 'guest'), now()->addSeconds(20), function () use ($user) {
            ...
            $avgGrade = round((float) Grade::query()
                ->when(! $isAdmin, fn ($q) => $q->whereIn('student_id', (clone $studentsBase)->pluck('id')))
                ->avg('score'), 1);
            ...
            $contentXpByUser = ContentProgress::query()
                ...
                ->when(! $isAdmin, fn ($q) => $q->whereIn('user_id', (clone $studentsBase)->pluck('user_id')))
                ->groupBy('user_id')
                ->pluck('xp', 'user_id');

            $quizXpByUser = LiveQuizAnswer::query()
                ->selectRaw('student_user_id as user_id, SUM(xp_earned) as xp')
                ->when(! $isAdmin, fn ($q) => $q->whereIn('student_user_id', (clone $studentsBase)->pluck('user_id')))
